...
class BallSortEnv(gym.Env):
    def __init__(self):
        ...
        self.observation_space = gym.spaces.Box(low = np.zeros((self.n_tubes, 4)), high=self.n_colors * np.ones((self.n_tubes, 4)), dtype = "float32")
    ...
    def step(self, action):
        ...
        elif self.is_solved_tube(source_tube):
            ilegal = True
            #print("ilegal: undoing a completed tube")
    ...
    def reset(self):
        self.state = np.zeros((self.n_tubes, 4), dtype="float32")
    ...
    def is_solved_tube(self, tube):
        if len(set(tube)) == 1 and tube[-1] != 0:
            return True
        else:
            return False
